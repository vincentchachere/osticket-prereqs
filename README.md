...
- If you dont see the image displayed below then try uninstalling and reinstalling IIS
  ...
  - right-click: `start` > click: `Run` > Type In: `Control` > Click: `Programs`>Click: `Turn Windows on or off` >
  - *Uncheck:* `IIS` > *Uncheck:* `World Wide Web Services` > *Uncheck:* `Application Development Features` (Inside World Wide Web Services) >
  - *Uncheck:* `CGI` (Application Development Features) > *Uncheck:* `Common HTTP Feautures` (Inside World Wide Web Services) >
  - *Uncheck:* `HTTP Redirection` (Inside Common HTTP Feautures) > *Uncheck:* `WebDAV Publishing` (Inside Common HTTP Feautures)
